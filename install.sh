...
_missing=""
for _cmd in mmdebstrap parted wipefs mkfs.fat mkfs.ext4 blkid \
            lsblk blockdev partprobe udevadm chroot grub-install \
            update-grub mount umount python3; do
...
if [ -n "$_missing" ]; then
    ...
    echo "  apt install mmdebstrap ${GRUB_PKG}-bin \\"
    echo "              dosfstools e2fsprogs parted util-linux python3"
    exit 1
fi
echo "  OK"
echo ""
